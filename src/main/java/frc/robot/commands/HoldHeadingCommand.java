...
public class HoldHeadingCommand extends Command {
  ...
  public void execute() {
    ...
    // Negate the output to correct the direction
    rawPIDOutput = -rawPIDOutput;
    ...
    SmartDashboard.putNumber("Raw PID Output", rawPIDOutput);
    SmartDashboard.putNumber("Clamped Output", rotationCorrection);
    SmartDashboard.putBoolean("Output Clamped", Math.abs(rawPIDOutput) > m_maxRotationSpeed);
  }
